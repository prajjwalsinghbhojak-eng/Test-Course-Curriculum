...
git ls-tree company/main --name-only
# Expected: _monitoring_app/ (containing monitor/ setup.sh setup.py .env.monitor.example Makefile ...) AI_CoE_Curriculum/ ...
...
cp _monitoring_app/.env.monitor.example .env.monitor
...
# 4. Run setup
bash _monitoring_app/setup.sh   # Mac/Linux/Codespaces
# python _monitoring_app/setup.py  # Windows

# 5. Verify
make -f _monitoring_app/Makefile health
```

Expected `make -f _monitoring_app/Makefile health` output — all lines should show ✓:
...
```bash
python -m pytest _monitoring_app/tests/ -q
```
...
```bash
make -f _monitoring_app/Makefile report
...
|---|---|---|
| `make -f _monitoring_app/Makefile health` shows daemon not running | setup.sh failed silently | `make -f _monitoring_app/Makefile restart` or check `logs/daemon.log` |
...
| `make -f _monitoring_app/Makefile sync` fails with 401/403 | Token wrong or expired | Check `MONITOR_PUSH_TOKEN` in `.env.monitor` |
| `make -f _monitoring_app/Makefile sync` fails with 404 | `learning-monitor-reports` repo doesn't exist | Complete step A1 |
...
| Tests failing | Import error | Run `python -m pytest _monitoring_app/tests/ -v` for details |
