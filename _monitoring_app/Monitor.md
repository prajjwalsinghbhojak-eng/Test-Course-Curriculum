...
```
PreToolUse   → python3 _monitoring_app/monitor/log_event.py --source claude
PostToolUse  → python3 _monitoring_app/monitor/log_event.py --source claude
Stop         → python3 _monitoring_app/monitor/log_session.py --source claude
Notification → python3 _monitoring_app/monitor/log_event.py --source claude
```
...
make -f _monitoring_app/Makefile setup                   # full bootstrap (idempotent)
make -f _monitoring_app/Makefile health                  # 6-point system check
make -f _monitoring_app/Makefile report                  # generate 7-day progress report
make -f _monitoring_app/Makefile sync                    # push logs to monitoring-reports
make -f _monitoring_app/Makefile status / stop / restart # daemon control
```
